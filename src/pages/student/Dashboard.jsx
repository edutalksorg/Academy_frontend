            ...
            {recentTests.map((attempt) => (
              <div key={attempt.id} className="flex justify-between items-center py-3 border-b last:border-b-0">
                ...
                <div className="text-right">
                  <div className={`text-lg font-bold ${attempt.totalScore >= 70 ? 'text-green-600' :
                      attempt.totalScore >= 50 ? 'text-yellow-600' :
                        'text-red-600'
                    }`}>
                    {attempt.totalScore}%
                  </div>
                  <div className="text-xs text-gray-500">
                    {attempt.totalScore >= 70 ? 'Excellent' :
                      attempt.totalScore >= 50 ? 'Good' :
                        'Needs Improvement'}
                  </div>
                </div>
              </div>
